...
class DebitConnectCore{
	...
	function setVOPAuftrag($status,$pkOrder)
	{
		...
		if($status == 101)
		{
			...
			if($exists['oldVOPStatus']=="0") return DC()->db->dbQuery("DELETE FROM dc_auftrag where id = ".(int) $pkOrder,false);
			$insert = new stdClass();
	...
	function Log($art,$txt,$errormsg = 0,$pkorder = 0,$exception = null)
	{
	...
	function init($smarty)
	{
		...
		if($this->cronJob == null) $this->cronJob	 = new DebitConnect_Cronjob(); 
		if($this->dataTypes == null) $this->dataTypes = new DC_DataTypes($this->db);
        if($this->API == null){
	...
	public function checkRegistered()
	{
		...
		if($check['activated'] == 1 && strlen($check["vopUser"])>0 && strlen($check["vopToken"])>5){ return true;
		}else {
	...
	public function sendMahnung($pkOrder)
	{
		...
		if(DC()->settings->currentSetting->shopwareapibenutzen==0) return false;
	...
	{
		$vopCMD = array("new" => "INSERT","update" => "UPDATE","delete" =>"DELETE" , "change" => "UPDATE"); 
		$soap = $this->API->mahnwesen();
	...
	// arr = soap.addNewChange(item.user, item.authCode, art.ToString(), rgNr.ToString(), cKorrekturNr.ToString(), dErstellt.ToString(), nZahlungsziel.ToString(), nInkassoStatus.ToString(), cBezahlt.ToString(), cKundenNr.ToString(), cFirma.ToString(), cAnrede.ToString(), cTitel.ToString(), cVorname.ToString(), cName.ToString(), cStrasse.ToString(), cPLZ.ToString(), cOrt.ToString(), cLand.ToString(), cTel.ToString(), cFax.ToString(), cEMail.ToString(), cMobil.ToString(), cZHaenden.ToString(), cGeburtstag.ToString(), nDebitorennr.ToString(), cAdressZusatz.ToString(), cSperre.ToString(), kRechnung.ToString(), betrag.ToString(), action.ToString());
		...
		$res =  $soap->addNewChange($vopParams["user"],$vopParams["pass"],$vopParams["art"],$vopParams["cRechnungsNr"],$vopParams["cKorrekturNr"],$vopParams["dErstellt"],$vopParams["nZahlungsziel"],$vopParams["nInkassoStatus"],
					$vopParams["cBezahlt"],$vopParams["cKundenNr"],$vopParams["cFirma"],$vopParams["cAnrede"],$vopParams["cTitel"],$vopParams["cVorname"],$vopParams["cName"],$vopParams["cStrasse"],$vopParams["cPLZ"],$vopParams["cOrt"],
						$vopParams["cLand"],$vopParams["cTel"],$vopParams["cFax"],$vopParams["cEMail"],$vopParams["cMobil"],$vopParams["cZHaenden"],$vopParams["cGeburtstag"],$vopParams["nDebitorennr"],$vopParams["cAdressZusatz"],$vopParams["cSperre"]
					,$vopParams["kRechnung"],$vopParams["Betrag"],$vopParams["cmd"]		);
				...
				try
				{
					...
					if($res->Error=="success")
					{
						...
						if(isset($status->gsoffen))
						{
							...
							if($auftrag["orderhash"]!=md5($status->hash)){ $update->orderhash = md5($status->hash);
							}
							$update->lastSync = date("Ymd");
							$this->db->dbUpdate("dc_status",$update,"pkOrder = ".(int)$auftrag["pkOrderAuftrag"],false);
							
							$newvopstatus = 0;
							if($status->art == 0 && $status->erledigt == 0) $newvopstatus = 55;
	...
	public function getOPListe()
	{
		...
		$dataType = $this->dataTypes->getOPListe($this->listView->startCount,$this->listView->maxCount,array(
		...
		$listview = array("header" =>  $this->listView->createListViewHeader(DC_SCRIPT,$headline), "table" => $this->listView->createList());
	...
	public function pageUserSettings($setting = "reg")
	{
		...
		if((DC()->hasvalue('setmatching')) && (DC()->hasvalue('savematching')))
		{
			...
			if(count($_regex['replace'])>0)
			{
				...
				{
					$regex[] = array($_regex['replace'][$i],$_regex['with'][$i],$_regex['comment'][$i]);
				}
			}
		...
		if((DC()->hasvalue('deleteBlacklist'))){
			if(DC()->get('deleteId')>=0){
				...
				DC()->settings->hbciBlacklist = json_decode($this->getConf("hbciBlacklist",json_encode(array()),true));
			}
		...
		// BLACKLISTE
			...
			if(strlen($_blacklist['art'])>0 && strlen($_blacklist['cString'])>0){
				$newEntry = array("art" => $_blacklist['art'],"cString" => $_blacklist['cString']);
				$blacklist[] = $newEntry;
				$this->setConf("hbciBlacklist",json_encode($blacklist),true);
				DC()->settings->hbciBlacklist = json_decode($this->getConf("hbciBlacklist",json_encode(array()),true));
			}
		...
		if((DC()->hasvalue('userregistration')))
		{
			...
			if(DC()->get('activate'))
			{
				...
				if($this->settings->registration["vopToken"] == $_reg['key'])
				{
					...
					}else $this->View('ERROR_MSG',"Ungültiger Aktivierungscode");
					}catch(Exception $e)
					{
		...
		foreach($this->settings->shopsArray as $shops)
		{
			...
			if($shops['activated'] == 0 ) $this->View('activated',1);
			if($shops['activated'] == 1 ) $this->View('activated',2);
			else $this->View('registered',true);
                        ...
                        if($profile->id == $selectedKonto[0] && $konto->IBAN == $selectedKonto[1]){
							...
							foreach(DC()->get('cbx') as $pkOrder){
								...
								if($dtaRow['amount'] > 0 && strlen($dtaRow['iban'])>0 && strlen($dtaRow['bic'])>0 && strlen($dtaRow['bankname'])>0 && 
									...
									$dtaXML->xmlFile->addTransfer( $payment_name, array(
	...
	//	$aktionsbtn = array("cssclass" => "button success fancyboxfullscreen","text" => "Übersicht","data-fancy-href" => DC_SCRIPT."?switchTo=overView&noncss=1&fancy=1&pkOrder=");
		...
		//$limitfilter = array($this->listView->startCount,$this->listView->maxCount);
		$this->listView->sessName = $type;
		$dataType = $this->dataTypes->getDTAList($this->listView->startCount,$this->listView->maxCount,array(
								"column"=> $this->listView->order,
								"direction" => $this->listView->orderDir),
								$this->listView->filter,$this->listView->fieldModes);
	...
	public function BoniGatewayLog(){
		...
		if($this->zalogdate== null) $this->zalogdate = date("d.m.Y");
		if((DC()->hasvalue('datefilter'))) $this->zalogdate =DC()->get('datefilter'); 
		...
		$this->listView->getCurrentOrder();
		$dataType["order"] =  array("PK" => array(false,"logtab.logid","Key",false),
									"customer_vname" => array(true,"logtab.customer_vname","Vorname",true),
									"customer_nname" => array(true,"logtab.customer_nname","Nachname",true),
									"customer_firma" => array(true,"logtab.customer_firma","Firma",true),
									"tstamp" => array(true,"logtab.tstamp","Datum",false),
									"zahlungsart" => array(true,"logtab.zahlungsart","Zahlungsart",true),
									"cArt" => array(true,"logtab.cArt","Art",true),
									"ResponseText" => array(true,"logtab.responseText","Antwort",false),
									"scoreinfo" => array(true,"logtab.scoreInfo","ScoreInfo",false),
									"cssclass" => array(false,"cssclass","cssclass",false),
									);
		...
		$listview = array("header" =>  $this->listView->createListViewHeader(DC_SCRIPT,"Bonitätsprüfung - Protokoll"), "table" => $this->listView->createList());
		$this->View("listview",$listview);
	...
	public function BoniGatewaySettings()
	{
		...
		foreach(DC()->dataTypes->getShippingMethods() as $shipping)
		{
			...
			try
					{
					$client = new SoapClient("https://api.eaponline.de/bonigateway.php?wsdl",array('cache_wsdl' => WSDL_CACHE_NONE));
	...
	if((DC()->hasvalue('newProfile')) && strlen(DC()->get('ProfileName'))>0){
		...
		$entry->profileData = json_encode(array());
	...
	public function getVOPBelege(){
		...
		$dataType = $this->dataTypes->getBelege($this->listView->startCount,$this->listView->maxCount,
								array("column"=> $this->listView->order,"direction" => $this->listView->orderDir),$this->listView->filter,$this->listView->fieldModes);
		...
		$this->listView->createUnsortedList($res,$dataType['count']);
		$listview = array("header" =>  $this->listView->createListViewHeader(DC_SCRIPT,"V.O.P Belege"), "table" => $this->listView->createList());
	...
	public function getDashBoard(){
	  ...
	  if(count($logEntrys)>0)  $this->smarty->assign("cronjob_log",$logEntrys);
	...
	public function akteneinsicht()
	{
		...
		if($fkto->Error != "Error")
		{
					$_fkto['hauptforderung'] = array("soll" => number_format(round($fkto->Hauptforderung,2),2,",","."),
					"haben" => number_format(round($fkto->ZEaufHaupt,2),2,",","."),
					"saldo" => number_format((round($fkto->Hauptforderung,2)-round($fkto->ZEaufHaupt,2)),2,",","."));
					
					$_fkto['zinsen'] = array("soll" => number_format(round($fkto->Zinsen,2),2,",","."),
					"haben" => number_format(round($fkto->ZEaufZinsen,2),2,",",".")
					,"saldo" => number_format((round($fkto->Zinsen,2)-round($fkto->ZEaufZinsen,2)),2,",","."));
					
					$_fkto['ra'] = array("soll" => number_format(round($fkto->Kostenverzinschlich,2),2,",","."),
					...
					$_fkto['kosten'] = array(
					"soll" => number_format(round($fkto->Kosten,2),2,",",".")
					,"haben" => number_format(round($fkto->ZEaufKosten,2),2,",",".")
					,"saldo" => number_format((round($fkto->Kosten,2)-round($fkto->ZEaufKosten,2)),2,",","."));
					$haben = number_format((round($fkto->ZEaufHaupt+$fkto->ZEaufZinsen+$fkto->ZEaufKostenVerzinslich+$fkto->ZEaufKosten,2)),2,",",".");;

					$_fkto['salden'] = array("soll" => number_format(round($fkto->Hauptforderung,2)+round($fkto->Zinsen,2)+round($fkto->Kostenverzinslich,2)+round($fkto->Kosten,2),2,",","."),
					"haben" => $haben,
	...
	public function zaLOG()
	{
		...
		$this->listView->getCurrentOrder();
		$dataType["order"] =  array("PK" => array(false,"umsatz.kUmsatz","Key",false),
									"name" => array(true,"umsatz.cName","Name",true),
									"dBuchung" => array(true,"umsatz.dBuchung","Datum ",false),
									"wert" => array(true,"umsatz.fWert","Betrag",false),
									"cVzweck" => array(true,"umsatz.cVzweck","Verwendungszweck",true),
									"sumGesamt" => array(false,"fWert","Betrag",false)
									);
		...
		$listview = array("header" =>  $this->listView->createListViewHeader(DC_SCRIPT,"Zahlungsabgleich - Protokoll"), "table" => $this->listView->createList());
		$this->View("listview",$listview);
	...
	public function logbuch()
	{
		...
		$res = $this->db->getSQLResults($dataType['query']);
		$this->listView->createUnsortedList($res,$dataType['count']);
		$listview = array("header" =>  $this->listView->createListViewHeader(DC_SCRIPT,"Logbuch"), "table" => $this->listView->createList());
	...
	public function zahlungsabgleichManuell()
	{
		...
		if((DC()->hasvalue('submitaction')))
		{
			...
			if((DC()->hasvalue('change')))
			{
				...
				}else if (DC()->hbci->matches[DC()->get('transaction')]['pos'][DC()->get('pkOrder')]->richtung == "-"){
		...
		if(DC()->get('submitaction')!='setVerbuchen' && strlen($SteuerDateifromSoap)>10 || ((DC()->hasvalue('submitSteuerDatei')) && isset($_FILES['steuerdatei']))){
			...
			if(number_format($umsatz["fWert"],2) != number_format($countSumSteuerdatei,2)) $this->View("SUM_MISSMATCH",$countSumSteuerdatei);
			foreach($xml->rechnung as $rechnung){
				...
				foreach($pkOrderArr as $pkOrder){
					...
					if(DC()->hbci->bestellungen[$pkOrder] != null){
						...
						$beleg = array("RechJahr" => (string)$rechnung->nRechJahr, "RechNr" => (string)$rechnung->nRechNr);
	...
	public function sendZahlungseingang($pkOrder,$type = 1)
	{
		...
		if($this->settings->currentHBCI["bestaetigung"]==0) return;
	...
	{	
		...
		if(DC()->settings->currentSetting->shopwareapibenutzen==0 && !$vorschau) return false;
		$this->setVOPAuftragDetail($pkOrder);
		$tpl = null;
		$params = $this->dataTypes->getZEVars((int)$pkOrder);
		foreach($params as $key => $value)
	...
	public function zaSuche()
	{
		...
		if((DC()->hasvalue('suchebestellung')))
		{
			...
			{
				if((DC()->hasvalue('limit')) && DC()->get('limit')=="open") if($bestellung["offen"]<=0) continue;
				
				if(count($directSearch)>0 && in_array($bestellung["id"],$directSearch)){
					$searchres[] = $bestellung;
				}else if(strtoupper($bestellung['RechnungsNr']) == strtoupper(DC()->get('searchfield')))
				{
					$searchres[] = $bestellung;
				}
				else if(strtoupper($bestellung['ordernumber']) == strtoupper( DC()->get('searchfield')))
				{$searchres[] = $bestellung;
				}
				else if(strtoupper($bestellung['firstname']) ==  strtoupper(DC()->get('searchfield')))
				{$searchres[] = $bestellung;
				}
				else if(strtoupper($bestellung['lastname']) ==  strtoupper(DC()->get('searchfield')))
				{$searchres[] = $bestellung;
				}
				else if(strtoupper($bestellung['KundenNr']) ==  strtoupper(DC()->get('searchfield')))
				{$searchres[] = $bestellung;
				}
			}			
			$this->View('searchres',$searchres);
	...
	public function setMahnstop()
	{
		if((DC()->hasvalue('changeMahnstop')))
		{
			...
			if((DC()->hasvalue('pkCustomer')))
			{
				...
				else if((DC()->hasvalue('removeMahnstopCustomer')))
				{
			...
			if((DC()->hasvalue('pkOrder')))
			{
				...
				else if((DC()->hasvalue('removeMahnstopOrder')))
				{
	...
	public function getOverViewGateway($pkCustomer){
		...
		if($this->boniGateway== null) $this->boniGateway = new DebitConnect_BoniGateway ;
		...
		$this->View("kennziffern",$this->boniGateway->kennziffern);
			...
			}else if(array_key_exists("trefferliste",$request))
	...
	function getCurrentPage()
	{
		...
		if($this->current_page == null) $this->current_page = "start";
	...
	function getConf($art,$standard,$global = false)
	{
		...
		if($res == null) {
			...
			$res= array("datavalue" => $standard);
		}
		return $res["datavalue"];
	}
	function setConf($art,$value,$global = false)
	{
		$shop = $global ? 0 :DC()->settings->selectedShop;
		$obj = new stdClass();
		$obj->datavalue = ($value);
		...
		function findkeyvalue($array, $key, $val)
	{
...
function doInstallOrUpdate($lastVersion)
{
...
function checkInstallation()
	...
	$dbVersion = $this->getDBVersion();
		if($updateRequired)
		{
			...
			echo json_encode(array("InstallationRequired"=> true));
			}
			else
			{
				...
				$this->View("SOFTWARELIZENZ",utf8_encode(file_get_contents("Softwarelizenzvertragsbedingungen.txt")));
				 print ( $this->smarty->fetch(dirname(__FILE__)."/../tpl/install.tpl"));
				}
				else
				{
