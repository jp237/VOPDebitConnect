...
class listView
{
    ...
    public $maxCountArr;
    public $columns;
    public $scriptName = 'VOPDebitConnect?';
    public $unsorted_list;
    public $customBtn;
    public $menuBtn;
    public $checkboxes;
    public $aktion;
    ...
    public function __construct($aktionbtn = false, $menubtn = false, $checkbox = false, $progressBar = false)
    {
        if (($aktionbtn != false)) {
    ...
    {
        $this->maxCountArr = [20, 50, 100, 300, 500, 1000];
    ...
    public function createUnsortedList($aDataType, $maxCounts)
    {
    ...
    public function createList()
    {
        ...
        if ($this->rsList) {
            foreach ($this->rsList as $singleRow) {
                ...
                if ($this->checkboxes) {
                    foreach ($singleRow as $key => $val) {// NUR ERSTEN INDEX AUS DEM ARRAY NEHMEN
                ...
                if ($this->aktion) {
                    $output .= '<td  align="left">' . $this->createCustomBtn($singleRow['id']) . '</td>';
                }
        ...
        if ($this->sumBetrag > 0 || $this->sumOffen > 0) {
            if ($this->sumBetrag > 0) {
                $summe .= ' Betrag : ' . number_format($this->sumBetrag, 2, ',', '.') . '   ';
            }
            if ($this->sumOffen > 0) {
                $summe .= ' Offen : ' . number_format($this->sumOffen, 2, ',', '.');
            }
    ...
    public function createProgressbar($width)
    {
        if ($width < 0) {
    ...
    public function createCustomBtn($id)
    {
        $btn = $this->customBtn;
        $a = '<a ';
        if (isset($btn['cssclass'])) {
        ...
        if (isset($btn['href'])) {
            $a .= " onclick='showLoader();' href='" . $btn['href'] . "$id' ";
        } elseif (isset($btn['data-fancy-href'])) {
            $a .= "data-fancybox-href='" . $btn['data-fancy-href'] . "$id' ";
        }
        $a .= '>' . $btn['text'] . '</a>';
    ...
    public function createHeader($db_field, $headertext, $fieldLength = 0, $fieldData = false, $fieldModeArr = false, $noBR = false, $db_field_sort = false, $showHeader = true)
    {
        $scriptName = $this->scriptName;

        global $scriptName;
        if ($db_field_sort) {
            $sortField = $db_field_sort;
        } else {
            $sortField = $db_field;
        }
        ...
        if ($showHeader) {
            $html = "<a href='" . $this->scriptName . '&cmd=changeOrder&changeto=' . $sortField . "' title='" . $linkTitle . "'" . $linkColor . '>' . $headertext . '</a>' . $linkDir;
        } else {
            $html = '';
        }
        if ($fieldModeArr) {
            ...
            $standardElement = ['id' => '', 'description' => ''];
            ...
            foreach ($fieldModeArr as $arrayRow) {
                ...
                $value = $showbez;
                if (isset($this->fieldModes[$db_field]) and $this->fieldModes[$db_field] == $value) {
                    $selected = ' selected';
                } else {
                    $selected = '';
                }
        ...
        if ($fieldLength and !$fieldData) {
            if (isset($this->filter[$db_field])) {
                $fieldValue = $this->filter[$db_field];
            } else {
                $fieldValue = '';
            }
            if (!$noBR) {
        ...
        if (!$fieldLength and $fieldData) {
            ...
            foreach ($fieldData as $value => $showbez) {
                if (isset($this->filter[$db_field]) and $this->filter[$db_field] == $value) {
                    $selected = ' selected';
                } else {
                    $selected = '';
                }
    ...
    public function createMaxCountSelector()
    {
    ...
    public function createListViewHeader($scriptName = 'VOPDebitConnect', $headline = 'DEFINE TITLE')
    {
        ...
        if ($this->checkboxes) {
            $header .= "<td><input class='checkall' type='checkbox' name='selectAll'></td>";
        }
        if ($this->progressBar) {
            $header .= '<td>Fortschritt</td>';
        }
        $exportHeaderRow = [];
        foreach ($this->columns as $key => $value) {
            if ($value[0] == true) {
    ...
    public function createPageSelector()
    {
        ...
        for ($i = 0; $i < $this->counts; $i += $this->maxCount) {
            if ($i >= $this->startCount and $i < ($this->startCount + $this->maxCount)) {
                $sel = ' selected';
            } else {
                $sel = '';
            }
        ...
        if ($this->startCount - $this->maxCount > $this->maxCount) {
        ...
        $changeto = $this->startCount + $this->maxCount;
        if ($this->startCount + $this->maxCount > $this->counts) {
            $dis = ' disabled';
        } else {
            $dis = '';
        }
        ...
        if ($this->startCount > $this->counts - $this->maxCount) {
            $dis = ' disabled';
        } else {
            $dis = '';
        }
    ...
    public function setStartCount($newStartCount)
    {
        $this->startCount = $newStartCount;
        $this->writeSession();
    }

    public function setMaxCount($newMaxCount)
    {
        $this->maxCount = $newMaxCount;
        $this->writeSession();
    }

    public function setFieldMode($fieldModes)
    {
    ...
    public function changeCat($whatcat)
    {
        $this->cat = $whatcat;
        $this->writeSession();
    }

    public function changeOrder($whatorder)
    {
    ...
    public function setFilter($fmFilter)
    {
        if (count($fmFilter) > 1) {
    ...
    public function delFilter($field)
    {
        unset($this->filter[$field]);
        $this->writeSession();
    }
    ...
    public function catSelected($whatcat)
    {
        if ($whatcat == $this->cat) {
            return ' selected';
        }
    ...
    public function getFormData()
    {
        ...
        if ($cmd == 'configListView') {
            ...
            if ($this->filter != $oldFilter) {
                $startCount = 0;
            }			// wenn sich der Filter ge�ndert hat, also die Anzahl der Suchergebnisse, dann setze Startseite auf 1
        ...
        if ($cmd == 'changeStartCount') {
            $this->setStartCount($changeto);
        }
        if ($cmd == 'changeOrder') {
            $this->changeOrder($changeto);
        }
        if ($cmd == 'changeCat') {
            $this->changeCat($changeto);
        }
        if ($cmd == 'resetFilter') {
            $this->resetFilter();
        }
        if ($cmd == 'cancelNachbearbeitung') {
            $this->cancelNachbearbeitung($changeto);
        }
    }

    public function getVar($varname, $varlen = 30)
    {
        ...
        return substr($returnvalue, 0, $varlen);
    }
}
