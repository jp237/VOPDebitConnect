...
class DebitConnect_Cronjob{
	...
	function Log($step,$result,$json,$isError = 0,$pkOrder = 0){
	    $entry = new stdClass();
	...
	function doTasks($jsonOutput = false){
			...
			foreach(DC()->db->getSQLResults("SELECT shopID from dc_firma where activated = 1") as $shop)
			{
                ...
                if($_SERVER["REMOTE_ADDR"] == "62.225.158.106" || DC()->hasvalue('debug')){
					...
					if(count($hbciProfiles)>0 && DC()->settings->cronjob->zahlungsabgleich>0 )
					{
						...
						if(DC()->hbci->verbucht>0) $syncLast = false;
						...
						if(!DC()->settings->cronjob->ze->active>0) $status[$firma]['zahlungserinnerung']['notactive'] = true;
						else
						{	
							if(count($oposData)>0)
							{
						...
						$oposList = DC()->dataTypes->getOPOSList(false,null,DC()->settings->currentSetting->statusZE,null
																					,39,DC()->settings->currentSetting->fristMA,null
																					,DC()->settings->cronjob->ma->kundengruppe,DC()->settings->cronjob->ma->minvalue,array(),true,null);
						$oposData = DC()->db->getSQLResults($oposList["query"]);
						$status[$firma]['mahnung']['list'] = count($oposData);
						if(!DC()->settings->cronjob->ma->active>0) $status[$firma]['mahnung']['notactive'] = true;
						else
						{
							if(count($oposData)>0)
							{
