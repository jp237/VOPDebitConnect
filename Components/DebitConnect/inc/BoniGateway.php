...
var $WSDL = "https://api.eaponline.de/bonigateway.php?wsdl";
var $userLogin;
var $userPass;
var $api;
var $projecte_b2b;
...
var $kennziffern;
	...
	function Log($art,$txt,$errormsg){
		DC()->log($art,$txt,$errormsg);
	}
	
	
	function initApi(){
		$this->api = new SoapClient($this->WSDL,array(  'encoding' => 'UTF-8', 'cache_wsdl' => WSDL_CACHE_NONE,'trace' => 1));
	}
	...
	public function getRequest(){
		...
		if(DC()->get('request_art') == "B2C"){
				...
				$ergebnis = array("ergebnis" => DC()->get('request_art'), "response" => $response);
			
			
		}else if(DC()->get('request_art') == "Kaskade"){
		...
		$ergebnis = array("ergebnis" => DC()->get('request_art'), "response" => $response);
		...
		}else if(DC()->get('request_art') == "Kompakt"){
			...
			if(DC()->get('request_id')){
				...
				$ergebnis = array("trefferliste" =>  true , "response" =>  $response);
			}
		}else if(DC()->get('request_art') == "Vollauskunft"){
			if(DC()->get('request_id')){
				$type = 0;
				$interesse = DC()->get('intresse');
				$request_id = DC()->get('request_id');
	...
	function checkLoginGateway($username,$pass){
		$this->initApi();
		$pass = md5($pass);
		$this->projecte_b2b = array();
		$this->projecte_b2c = array();
			...
			if($wsresponse->status == "True"){
				...
				foreach($projectresponse as $project){
					if(!isset($project->Error)){
						...
						}else if($project->projecttype == "B2B"){
