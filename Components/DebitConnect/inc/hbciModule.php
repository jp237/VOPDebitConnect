...
class HBCI_MODULE{
	...
	function setHBCIData($url,$blz,$alias,$pin)
	{
	...
	function setHBCIProfile($profileObject){
	...
	{
		$values = array( 0 => "Rechnungsbetrag",1 => "Mahnkosten",2 => "Skonto" , 3 => "Überzahlung",4 => "Erstattung",5 => "Bankrücklastkosten" , 7 => "Bankrückbelastung" , 8 => "Gutschrift"  );
		return $values[$input];
	}
	
	function getVWZ($iban){
	...
	function checkPaymentMahngebuehr($pkOrder){
		$mahngeb = 0.00;
	...
	function getPaymentDetail($pkOrder = 0)
	{
	...
	function getCountPaymentsReady()
	{
		$count = 0;
	...
	function matchDTA($kUmsatz,$idDta){
		if($this->matchedDTA == null){
			$this->matchedDTA = array();
		}
		$this->matchedDTA[$kUmsatz] = $idDta;

	}

	function writeBackUmsatz($ajaxwriteback = false)
	{
		if(count($this->matches)>0)
		{
			foreach($this->matches as $kUmsatz => $match)
			{
				if($match["verbuchen"] && !$match["verbucht"])
				{
					...
					if($verbucht)
					{
						...
						else DC()->db->dbQuery("DELETE from dc_tzahlung where kUmsatz = ".(int) $kUmsatz);
	...
	function resetKreditLimit($value,$ordernumber,$pkOrder){
	try
		{
			...
			if(strlen($username)>0 && strlen($passwd) == 32 && $passwd != "d41d8cd98f00b204e9800998ecf8427e"){
				$value = ceil($value);				
				$soap = new SoapClient('https://webservice.eaponline.de/webservice.php?wsdl',array(  'encoding' => 'UTF-8', 'cache_wsdl' => WSDL_CACHE_NONE,'trace' => 1));
	...
	function checkKomplettBezahlt($umsatz)
	{
	...
	function writeBackPaymentHistory($umsatz,$match,$type = 0){
	...
	function flushdata()
	{
		...
		$this->verbucht = 0;
		$this->verbuchtsum = 0;
		$this->halfPayed = array();
		$this->entrys = 0 ;
		$this->skontoSKR = array();
	}
	function UmsaetzeFromDB($cronjobMode = false,$cronJobLimit = 30)
	{//cast(dBuchung AS DATE) DESC ,
		...
		if(DC()->settings->currentHBCI["zahlungsausgang"] == 1) $type = "(nType = 0)";
	...
	function getZahlungenTemplate($pkOrder)
	{
	...
	function regExPerson($ref,$compare,$punkte,$debug = false,$doublePointsMinStringLength = 0)
	{
		$debug = false;
		$copy_ref_orgi = $ref;
		if(strlen($ref)<3) return 0;
		if(strlen($compare)<3) return 0;
		
		$matched = 0;
		$ref = strtoupper($ref);
		$compare = strtoupper($compare);
		if($matched == 0 && $ref == $compare) $matched = $punkte; 
		if($matched == 0 && preg_match("/$compare/",$ref)) $matched = $punkte;
		if($debug)
		...
		foreach($regex as $reg)
		{
			...
			if($debug){ echo "REGEX >>>> ".$copyref." << $compare = $matched <br>";}
		}
	...
	function clearchars($inputstring)
	{
	...
	function regExAuftragData($ref,$compare,$debug = false)
	{
		
	}
	
	function matchBetrag($umsatz,$bestellung,$punkte=5)
	{
		...
		if($umsatz["nType"]==1){
			...
			}else if($bestellung['offen'] <= 0 && $fWertPositiv > $bestellung['betrag']){
		...
		catch(Exception $d)
		{
			...
			*/
			...
			else if($umsatz['fWert'] == $bestellung['offen']+DC()->settings->currentSetting->mahngeb)
			{
				$return['punkte']=$punkte;
				$return["fWert"] = $bestellung['offen'];
				$return['mahngeb'] = DC()->settings->currentSetting->mahngeb;
			}
			else if($skonto && $sumSkonto == $umsatz['fWert'] )
			{
				$return['punkte']=$punkte;
				$return["fWert"] = $sumSkonto;
				$return['skonto'] = $skontobetrag;
			}
			else if($bestellung['offen']<$umsatz['fWert'])
			{
				$return["fWert"] = $bestellung['offen'];
			}
			else if($bestellung['offen']>$umsatz['fWert'])
	...
	function setUmsatzBlackliste($kUmsatz){
		$update = new stdClass();
		$update->nType = 10 ;
		return DC()->db->dbUpdate("dc_umsatz",$update,"kUmsatz = ".(int)$kUmsatz);
	}
	
	function addMatching($kUmsatz,$pkOrder)
	{
		$umsatz = $this->umsaetze[$kUmsatz];
	...
	function getMatching($ajaxSingleSync = 0,$cronJob = false)
	{
		...
		{  
			...
			// SINGLE SYNC, 
				continue;
			}else if ($ajaxSingleSync>0 && $count> $ajaxSingleSync)
			{
				break;
				//BREAK
			}
			...
			foreach(DC()->settings->hbciBlacklist as $blackListEntry){
				...
				if($blackListEntry->art == 0 && strpos(strtoupper($umsatz["cName"]),strtoupper($blackListEntry->cString)) !== false){
						$foundBlacklist = true;
				}else if($blackListEntry->art==1 && strpos(strtoupper($umsatz["cVzweck"]),strtoupper($blackListEntry->cString)) !== false) {
						$foundBlacklist = true;
				}
			}
			...
			$matching = 0;
			if($this->matches[$umsatz["kUmsatz"]]["verbucht"] == true ) continue;
			...
			foreach(@$this->bestellungen as $bestellung)
			{
				...
				if($bestellung["offen"]<=0 && $umsatz["nType"] == 0) continue;
				...
				else if ($matching >= $aehnlich)
				{
					
					if(!$selected ) $this->matches[$umsatz["kUmsatz"]]['selected'] = $bestellung["id"];
	...
	function returnBuchungsBetrag($umsatz)
	{
		$val = 0.00;
	...
	function returnCountBuchungPos($umsatz)
	{
		$val = 0.00;
		$returnArray= array("value" => 0.00, "class" => null, "action" => false);
		...
		else if($umsatz["fWert"] != $returnArray["value"] && $umsatz['nType'] == 0) 
		{ 
			...
			else $returnArray["class"] = "orange";
		}
		else if((($umsatz["fWert"]*-1) != $returnArray["value"] && $umsatz['nType'] == 1 )){
			$this->matches[$umsatz["kUmsatz"]]["verbuchen"] = false;
		...
		}else if((($umsatz["fWert"]*-1) == $returnArray["value"] && $umsatz['nType'] == 1 ) || (($umsatz["fWert"]) == $returnArray["value"] && $umsatz['nType'] == 0) )	{
	...
	function abrufUmsatzCSV($delimiter = ";",$enclosure = '', $escape = "",$filename)
	{
	...
	if(strlen($filename)>0)
	{
		if(strtoupper(substr($filename,-3))=="CSV")
		{
			...
			if($imported>0) rename($fileFolder.$filename,$fileFolderLog.$filename);
		}
	}
}

	
	function abrufUmsatz($profileId,$selectedKonto,$from,$to)
	{
		DC()->setIgnoreAbort();
	...
	function returnKonten($profileObject)
	{
		$this->setHBCIProfile($profileObject);
		$ret = array();
		$konten = $this->hbci->getSEPAAccounts();
...
class buchungsPos{
	...
	var $Offen;
	var $Zahlbetrag = "0.00";
	var $Ueberzahlung = "0.00";
	var $skonto = "0.00";
	var $mahnkosten = "0.00";
	var $richtung;
	var $bankruecklastkosten = "0.00";
	var $bankruecklast  = "0.00";
	var $erstattung  = "0.00";
	var $gutschrift  = "0.00";
	var $verbucht = false;
	var $bestellung;
	var $steuererstattung = "0.00";
	
	var $matchedfirma = null;
	var $matchedfirstname = null;
	var $matchedlastname = null;
	var $matchedrechnungsnr = null;
	var $matchedauftragsnr = null;
	var $matchedbetrag = null;
	var $matchedskonto = null;
	var $matchedkundennr = null;
	var $matchedvalue = 0;
	var $vopUmsatz = false;
	var $vopBeleg = null;
	
	function __construct($pkOrder,$zugeordnet,$umsatz,$bestellung,$matchBetrag,$matchedvalue,$vopUmsatz = false,$beleg = null)
	{
		...
		if($this->richtung=="+" && $this->Offen < 0  && !$vopUmsatz) $this->Zahlbetrag = "0.00";
		if($matchBetrag['mahngeb']>0 || $matchBetrag['mahngeb'] < 0 ) $this->mahnkosten = number_format($matchBetrag['mahngeb'],2,'.','');
		if($matchBetrag['skonto']>0) $this->skonto = number_format($matchBetrag['skonto'],2,'.','');
		if($matchBetrag['bankruecklast']>0)  $this->bankruecklast = number_format($matchBetrag['bankruecklast'],2,'.','');
		if($matchBetrag['bankruecklastkosten']>0)  $this->bankruecklastkosten = number_format($matchBetrag['bankruecklastkosten'],2,'.','');
		if($matchBetrag['gutschrift']>0)  $this->gutschrift = number_format($matchBetrag['gutschrift'],2,'.','');
		if($matchBetrag['erstattung']>0)  $this->erstattung = number_format($matchBetrag['erstattung'],2,'.','');
		if($matchBetrag['steuererstattung']>0) $this->steuererstattung =  number_format($matchBetrag['steuererstattung'],2,'.','');
