...
<tr><td><input type="text" class='form-control' id='appendsearchfield' required name="searchfield" value='{$smarty.post.searchfield}' placeholder='Bitte Suchparameter eingeben'></td><td><input class='btn btn-success'  type="submit" name="Suchen"></td></tr>
<tr>
  <td>Verwendungszweck : 
  {assign var="keywords" value=" "|explode:{$smarty.get.vwz}}
    {foreach from=$keywords item=keyword}
        {if ($keyword|trim)|strlen >2}
             <a onclick="$('#appendsearchfield').val('{$keyword}');$('#searchform').submit();" class='btn btn-info' href='#'>{$keyword|trim}</a>
        {/if}
    {/foreach}
 </td>
  <td>&nbsp;</td>
...
<thead>
<tr><td><input type="checkbox" id="checkAll"> </td><td>Vorname</td><td>Nachname</td><td>Betrag</td><td>Offen</td><td>Rechnungsnummer</td><td>Auftragsnummer</td><td>Kundennummer</td><td>BestellDatum</td></tr></thead>
...
<input  class='btn btn-success' type='submit' name='submitbtn' value='Hinzufügen'>
</form>
