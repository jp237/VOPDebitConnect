...
class Frontend implements  SubscriberInterface
{
    ...
    return [
        'Enlight_Controller_Dispatcher_ControllerPath_Frontend_Adressvalidation' => array('GetAdressValidationController',4000),
        'Enlight_Controller_Action_PostDispatch_Frontend_Checkout' => array('onPostDispatch',300000),
        'Enlight_Controller_Action_PostDispatch_Frontend_Account' => array('onPostDispatch',300000),
    ];
}


    public function GetAdressValidationController(\Enlight_Event_EventArgs $args ){
        return __DIR__ . '/../Controllers/Frontend/Adressvalidation.php';
    }
	...
	public function onPostDispatch(\Enlight_Controller_ActionEventArgs $arguments) {
		...
		if ( $response->isException() || $request->getModuleName() != 'frontend' || !$view->hasTemplate()) {
			return;
		}
		...
		if($EAPBoniGateway->postdirekt == null){
		    $EAPBoniGateway->postdirekt = new \eap_postdirekt();
        }
        $shipping =   $request->getParam('eap_shipping_address_postdirekt_type');
        $invoice =  $request->getParam('eap_invoice_address_postdirekt_type');
        if($invoice>=0){
          //  print_r($_POST);
        //    exit;
        }
        if($EAPBoniGateway->postdirekt->response != null){
          $shipping =   $request->getParam('eap_shipping_address_postdirekt_type');
          $invoice =  $request->getParam('eap_invoice_address_postdirekt_type');
          if($invoice == "0"){
              $EAPBoniGateway->postdirekt->invoice_adress_overridden = true;
          }
            if($shipping == "0"){
                $EAPBoniGateway->postdirekt->shipping_adress_overriden = true;
            }
        }
        ...
        $EAPBoniGateway->postdirekt->response = Shopware()->Session()->eap_adressvalidation;
		$EAPBoniGateway->schufaBoni->responseData = null;
		if($EAPBoniGateway->postdirekt->response != null){
           $orderId =  Shopware()->Db()->fetchOne("SELECT id  from s_order where ordernumber  = '".$sOrderNumber."'");
            $comment =  Shopware()->Db()->fetchOne("SELECT internalcomment  from s_order where $orderId  = ".$orderId);

            if(count($EAPBoniGateway->postdirekt->response["billing"])>0) {
                $comment .= "  Postdirekt Rechnungsadresse : " . implode(" ", $EAPBoniGateway->postdirekt->response["billing"]);
            }
            if(count($EAPBoniGateway->postdirekt->response["shipping"])>0) {
                $comment .= "  Postdirekt Lieferadresse : " . implode(" ", $EAPBoniGateway->postdirekt->response["shipping"]);
            }

            if(strlen($comment)>0) {
                Shopware()->Db()->executeUpdate(" UPDATE s_order SET internalcomment = :comment WHERE id = :orderid",
                    array(
                        ':orderid' => $orderId,
                        ':comment' => $comment
                    ));
            }
           $EAPBoniGateway->postdirekt->response = null;
        }
		...
		// EXECUTE SCHUFA IDENT//

            if($EAPBoniGateway->postdirekt != null){
                if($EAPBoniGateway->postdirekt->isAccepted === false){
                    $EAPBoniGateway->smarty->assign('requestParams',$EAPBoniGateway->requestParams);
                    $EAPBoniGateway->smarty->assign('postdirekt',$EAPBoniGateway->postdirekt);
                }
            }
